...
import {
  OR_KEY_MATERIAL_UNPINNED_SALT_ROTATED_REASON,
...
export function humanizeOrDisabledReason(reason: string): string {
  ...
  // Matched against the exported constant, not a free-text substring: see
  // that constant's comment in or-key-material.ts for why a second copy of
  // the sentence (typed or matched) is the failure mode this guards against.
