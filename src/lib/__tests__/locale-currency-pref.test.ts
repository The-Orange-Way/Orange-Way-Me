...
describe("formatCurrencyPref — Bitcoin honours btcDisplayMode", () => {
  ...
  it("a whole-number BTC balance with no unit stamp is misread by the magnitude heuristic (the bug)", () => {
    ...
    // it otherwise. Call sites that know the row is stamped pass
    // unitIsExact instead (see OWM-T0751 for GoalDetailPage/GoalFormDialog).
    expect(formatCurrencyPref(1, "BTC", "us", "btc")).toBe("0.00000001 BTC");
  });

  it("unitIsExact stops a whole-number BTC balance being read as 1 satoshi", () => {
