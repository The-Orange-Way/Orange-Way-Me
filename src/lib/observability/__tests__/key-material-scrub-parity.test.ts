...
describe("key-material inventory: both scrubbers", () => {
  ...
  it.each(KEY_MATERIAL_FIELDS.map((f) => [f]))("PostHog redacts a property named %s", (field) => {
...
describe("key-material inventory: anchored names do not over-match", () => {
  // "pin" is matched as a whole word. As a bare substring it also matches
  // shipping_address and spinner_state; blanking those costs analytics and
  // incident-response signal for no security gain, which is the same class of
  // harm as the leak, one direction over.
  it("PostHog does not redact words that merely contain pin", () => {
    const r = event({
  ...
  // "salt" is NOT anchored, and this case pins the over-match that follows
  // from that rather than leaving it undocumented.
  //
  // This assertion used to read basalt_theme === "dark", which was wrong on
  // the branch it was written on: the inventory lists "salt" as a plain
  // substring and the analytics client's own hint list does too, so
  // basalt_theme was always blanked. The test asserted a precision neither
  // list provides, which is worse than asserting nothing, because a reader
  // takes it as a statement about the scrubber.
  //
  // Fixing it by anchoring "salt" was the other option and was rejected here:
  // that narrows an existing redaction, which can only ever send MORE data to
  // a vendor, and it does not belong in a change whose whole point is to send
  // less. It is worth its own look. Until then, one theme name is blanked in
  // analytics and nothing else is affected.
  it("PostHog blanks basalt_theme too, the accepted cost of an unanchored salt", () => {
    const r = event({ basalt_theme: "dark" });

    expect(r?.properties.basalt_theme).toBe("[redacted]");
  });
