 ...
 * that lost its types at a boundary: an untyped consumer, or one computing the
 * flag from a lookup that yields `boolean | undefined`. That is the only way
 * this call shape can now occur, and it is exactly the shape that used to fall
 * through to derive-and-pin, so it needs a test the compiler cannot delete.
 */
...
describe("planOrKeyMaterial", () => {
  ...
  /**
   * PostgREST returns a Postgres `numeric` as a string and only the integer
   * types as a number. The migration declares this column `integer`, so this
   * cannot bite today, but reading a transported number as "nothing is pinned"
   * would send a pinned row down derive-and-pin, which is the silent
   * destruction the whole module exists to prevent. Cheap contract,
  ...
  /**
   * The defect this change exists for. Silence is not a claim that the salt is
  ...
  it("refuses when the caller did not state whether the salt matches", () => {
    ...
    expect(planWithoutTypes(EMPTY, "current-salt").mode).toBe("refuse");
    expect(planWithoutTypes(EMPTY, "current-salt", { saltMatchesExistingRows: undefined }).mode).toBe(
      "refuse",
    );
  });
   ...
   * already pinned needs no statement about the salt, because it is not
   * deriving.
   */
