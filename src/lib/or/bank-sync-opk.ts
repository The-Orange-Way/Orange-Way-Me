 ...
 *
 * A local loop rather than a call to transactions-page.ts's
 * nextTransactionsPage: that helper's row guard requires external_id to be
 * a non-null string, and this endpoint's own EncryptedTxRow type allows
 * external_id: string | null. Routing through the stricter guard would
 * silently drop any row that arrived without an external_id, which is
 * exactly the silent-data-loss shape this fix exists to close.
 */
