 ...
 * change is the pre rotation salt: the one existing rows were sealed under.
 * It is read from the row on purpose and is deliberately NOT a parameter. A
 * salt argument would let a caller hand in the newly minted one, which is the
...
/**
 * Decide, and if the answer is "pin", compute the three OR columns.
 *
...
export async function computeOrPinColumns({
  ...
  // commit message: OWM-T0262 requires the suite to be seen red before it is
  // believed, and this seat has no local test runner. Minting a salt here is
  // the hazard the helper exists to remove, expressed in one line.
  ...
  if (orPlan.mode === "refuse") {
    // Write no OR column and let the caller proceed. In both refusal cases the
