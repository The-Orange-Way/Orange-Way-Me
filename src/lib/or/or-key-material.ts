 ...
 * actual remedy.
 *
 * Exported so friendly-error.ts's substring match, and the test that guards
 * it, both read this value instead of each holding their own copy of the
 * sentence. Two copies is how the match silently breaks: reword one and nothing
 * fails until a customer report.
 */
