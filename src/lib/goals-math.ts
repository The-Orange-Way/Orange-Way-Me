...
export function balanceHistory(
  ...
  const acctInfo = new Map(
    accounts.map((a) => [a.id, { currency: a.currency, formatVersion: a.format_version }]),
  );
  ...
  for (let i = months.length - 1; i >= 0; i--) {
    ...
    for (const t of inRange) {
      if (new Date(t.date + "T00:00:00").getTime() >= boundary) {
        const info = acctInfo.get(t.account_id);
        const cur = info?.currency ?? primaryCurrency;
        flowAfter += convert(Number(t.amount) || 0, cur, primaryCurrency, {
          unitIsExact: unitIsExact(info?.formatVersion),
        });
      }
    }
