...
export function VaultProvider({ children }: { children: React.ReactNode }) {
  ...
  const changeVaultPassword = useCallback(async (currentPassword: string, newPassword: string) => {
    ...
    // computeOrPinColumns takes the ROW and reads that salt off it. It has no
    // salt parameter at all, so the newSalt minted below is not in its reach
    // and the ordering fault is not representable rather than merely watched.
    // It returns null when the row is already pinned and when the stored state
    // cannot be used, so nothing is written in either case.
