  ...
  it("excludes an errored single-connection sync from the import bridge (OW-T0263)", () => {
    ...
    expect(
      ...
      "handleSync's import-bridge guard no longer excludes a connection whose own " +
        "press just errored. handleSyncAll gates the same call on !c.error; this " +
        "block's own docstring above describes the intended condition as 'processed " +
        "and no error' for BOTH call sites, so the two guards must not read " +
        "differently. Found: " +
        JSON.stringify(condition.trim()),
    ).toBe(true);
  });
