...
#
# WIDENED 2026-09-05. Two refusal paths had no case: a curl transport
# failure (the stub always exited 0, so CURL_RC was always 0), and a non-2xx
# HTTP status (the stub already honoured OW_TEST_HTTP_CODE, but no case ever
# set it). Both are covered now. The suite also asserted nothing about the
# request itself: nothing checked the URL carried the right project ref, or
# that the SQL body still filtered on the public schema and on
# security-definer functions. The stub now asserts both against
...
#     no case ever set it, so the 2xx-widening added previously had never
