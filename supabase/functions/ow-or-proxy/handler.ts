/**
 * ow-or-proxy request handling, lifted out of index.ts so it can be called
 * directly from vitest (OWM-T0231). index.ts still owns every real
 * dependency -- the esm.sh supabase-js import, the module-scope env reads,
 * the service-role client -- and builds them once, then passes them in here
 * as OrProxyDeps. This file imports nothing that only runs under Deno, so
 * the same code path that runs in production is the one the tests exercise,
 * rather than a parallel re-implementation that could drift from it.
 *
 * This is an extraction, not a rewrite: the request-handling logic below is
 * unchanged from the Deno.serve callback it was lifted from. See index.ts
 * for the full endpoint-by-endpoint behavioural comments; they were not
...
// "see my transactions" workflow many times over. The limit exists to stop
// a compromised user account from being used to flood OR's platform
// endpoints (or to abuse our quota).
 ...
 */
export const OR_REGION_HEADER = "us-east-1";

/**
 * Everything the handler needs that index.ts currently reads from module
 * scope or builds via the esm.sh import. Passed in rather than imported so
 * this file has no Deno-only or network-fetched dependency of its own.
 */
export interface OrProxyDeps {
  serviceClient: SupabaseClient;
  createUserClient: (authHeader: string) => SupabaseClient;
  orSupabaseUrl: string | null;
...
async function callOr(
  ...
  return await deps.fetchImpl(`${deps.orSupabaseUrl}/functions/v1/${endpoint}`, {
    method: "POST",
    headers: {
...
export async function handleOrProxyRequest(req: Request, deps: OrProxyDeps): Promise<Response> {
