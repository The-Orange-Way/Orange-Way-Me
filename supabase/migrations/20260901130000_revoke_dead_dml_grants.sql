...
--
-- ONE BEHAVIOUR CHANGE WORTH NAMING, on beta_allowlist. It is the only table
-- here losing SELECT for authenticated. A dead SELECT cell today answers with
-- zero rows because row level security filters them all out; after this it
-- answers "permission denied" instead. Nothing reads this table as authenticated
-- (it is read through the SECURITY DEFINER function is_email_in_beta_allowlist,
-- which runs as its owner and is unaffected), so the change is from one denial
-- to a different denial. It is called out because a reader comparing the two
-- errors should not mistake the second for a regression this migration caused.
--
