...
-- tables. "Dead" means: row level security is enabled, no restrictive
-- policies exist anywhere in schema public, and no permissive policy
-- targeting authenticated (or PUBLIC) is reachable for that command, so
-- the grant being removed changes zero rows returned today. Full
-- per-table reachable-command matrix is posted on OWM-T0489, re-verified
-- live immediately before this file was written.
--
...
--   handled separately with a security review before merge.
--
